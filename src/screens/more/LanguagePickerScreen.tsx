...
type Opt = { value: AppLang; label: string; flag: string };
      ...
      <View style={styles.container}>
        <View style={styles.header}>
          <View style={styles.headerIcon}>
            <Ionicons name="language-outline" size={18} color={styles._accent.color} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.h1}>{t("language")}</Text>
            <Text style={styles.sub}>{t("chooseLanguage")}</Text>
          </View>
        </View>
          ...
          {opts.map((o, idx) => {
            ...
            return (
              ...
              >
                ...
                <View style={[styles.dot, active && styles.dotActive]}>{active ? <View style={styles.dotInner} /> : null}</View>
              </Pressable>
            );
          })}
        </View>
        ...
        <View style={styles.tip}>
          <Ionicons name="information-circle-outline" size={16} color={styles._sub.color} />
          <Text style={styles.tipText}>Мову можна змінити в будь-який момент.</Text>
        </View>
      </View>
    </SafeAreaView>
  );
...
function createStyles(colors: any, isDark: boolean) {
  ...
  const card = colors?.card ?? (isDark ? "rgba(21,24,27,0.72)" : "rgba(255,255,255,0.88)");
  const soft = isDark ? "rgba(47,111,78,0.22)" : "rgba(47,111,78,0.14)";
  ...
  return StyleSheet.create({
    ...
    bgOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: isDark ? "rgba(0,0,0,0.16)" : "rgba(255,255,255,0.18)" },
    ...
    sub: { marginTop: 4, fontSize: 12, color: sub, fontFamily: "Manrope_600SemiBold" },
    ...
    flagWrapActive: { borderColor: accent, backgroundColor: soft },
    flag: { fontSize: 16 },

    rowTitle: { fontSize: 14, color: text, fontFamily: "Manrope_700Bold" },
    ...
    dotActive: { borderColor: accent, backgroundColor: soft },
    dotInner: { width: 10, height: 10, borderRadius: 999, backgroundColor: accent },
    ...
    tipText: { flex: 1, fontSize: 12, color: sub, fontFamily: "Manrope_600SemiBold", lineHeight: 16 },
  });
}